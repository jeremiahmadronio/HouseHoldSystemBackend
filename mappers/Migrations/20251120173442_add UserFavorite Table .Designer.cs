// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using WebApplication2.data;

#nullable disable

namespace WebApplication2.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20251120173442_add UserFavorite Table ")]
    partial class addUserFavoriteTable
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("WebApplication2.models.Admin", b =>
                {
                    b.Property<Guid>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("password")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("id");

                    b.ToTable("Admins");
                });

            modelBuilder.Entity("WebApplication2.models.BudgetPlan", b =>
                {
                    b.Property<int>("BudgetPlanId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("BudgetPlanId"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("Days")
                        .HasColumnType("integer");

                    b.Property<string>("DietaryTag")
                        .HasColumnType("text");

                    b.Property<int>("Members")
                        .HasColumnType("integer");

                    b.Property<decimal>("TotalBudget")
                        .HasColumnType("numeric");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("BudgetPlanId");

                    b.HasIndex("UserId");

                    b.ToTable("BudgetPlans");
                });

            modelBuilder.Entity("WebApplication2.models.BudgetPlanItem", b =>
                {
                    b.Property<int>("BudgetPlanItemId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("BudgetPlanItemId"));

                    b.Property<int>("BudgetPlanId")
                        .HasColumnType("integer");

                    b.Property<int>("CommodityId")
                        .HasColumnType("integer");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("numeric");

                    b.HasKey("BudgetPlanItemId");

                    b.HasIndex("BudgetPlanId");

                    b.HasIndex("CommodityId");

                    b.ToTable("BudgetPlanItems");
                });

            modelBuilder.Entity("WebApplication2.models.Commodity", b =>
                {
                    b.Property<int>("CommodityId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("CommodityId"));

                    b.Property<string>("Category")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("LocalName")
                        .HasColumnType("text");

                    b.Property<string>("ProductName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Specification")
                        .HasColumnType("text");

                    b.HasKey("CommodityId");

                    b.ToTable("Commodities");
                });

            modelBuilder.Entity("WebApplication2.models.DietaryTag", b =>
                {
                    b.Property<int>("DietaryTagId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("DietaryTagId"));

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.HasKey("DietaryTagId");

                    b.ToTable("DietaryTags");
                });

            modelBuilder.Entity("WebApplication2.models.Games", b =>
                {
                    b.Property<Guid>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("correct_price")
                        .HasColumnType("integer");

                    b.Property<byte[]>("photo")
                        .HasColumnType("bytea");

                    b.Property<string>("product_name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("unit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("id");

                    b.ToTable("Games");
                });

            modelBuilder.Entity("WebApplication2.models.Market", b =>
                {
                    b.Property<int>("MarketId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("MarketId"));

                    b.Property<TimeSpan>("ClosingTime")
                        .HasColumnType("interval");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("MarketDescription")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("MarketName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<TimeSpan>("OpeningTime")
                        .HasColumnType("interval");

                    b.Property<string>("Region")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("ratings")
                        .HasColumnType("numeric");

                    b.HasKey("MarketId");

                    b.ToTable("Markets");
                });

            modelBuilder.Entity("WebApplication2.models.MealSuggestion", b =>
                {
                    b.Property<int>("MealSuggestionId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("MealSuggestionId"));

                    b.Property<int>("BudgetPlanId")
                        .HasColumnType("integer");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("MealSuggestionId");

                    b.HasIndex("BudgetPlanId");

                    b.ToTable("MealSuggestions");
                });

            modelBuilder.Entity("WebApplication2.models.PriceReport", b =>
                {
                    b.Property<int>("ReportId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("ReportId"));

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("ReportWeek")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("UploadDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UploadedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("ReportId");

                    b.ToTable("PriceReports");
                });

            modelBuilder.Entity("WebApplication2.models.ProductDietaryTag", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("CommodityId")
                        .HasColumnType("integer");

                    b.Property<int?>("DietaryTagId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("DietaryTagId");

                    b.HasIndex("CommodityId", "DietaryTagId")
                        .IsUnique();

                    b.ToTable("ProductDietaryTags");
                });

            modelBuilder.Entity("WebApplication2.models.ProductPrice", b =>
                {
                    b.Property<int>("ProductPriceId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("ProductPriceId"));

                    b.Property<int>("CommodityId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("DateReported")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int?>("MarketId")
                        .HasColumnType("integer");

                    b.Property<decimal>("Price")
                        .HasColumnType("numeric");

                    b.Property<int>("ReportId")
                        .HasColumnType("integer");

                    b.Property<string>("unit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("ProductPriceId");

                    b.HasIndex("CommodityId");

                    b.HasIndex("MarketId");

                    b.HasIndex("ReportId");

                    b.ToTable("ProductPrices");
                });

            modelBuilder.Entity("WebApplication2.models.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("email")
                        .HasColumnType("text");

                    b.Property<string>("password")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("phone")
                        .HasColumnType("text");

                    b.Property<byte[]>("photo")
                        .HasColumnType("bytea");

                    b.Property<string>("username")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("email")
                        .IsUnique();

                    b.ToTable("users", (string)null);
                });

            modelBuilder.Entity("WebApplication2.models.UserFavorite", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("CommodityId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("CommodityId");

                    b.HasIndex("UserId");

                    b.ToTable("UserFavorites");
                });

            modelBuilder.Entity("WebApplication2.models.BudgetPlan", b =>
                {
                    b.HasOne("WebApplication2.models.User", "User")
                        .WithMany("BudgetPlans")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("WebApplication2.models.BudgetPlanItem", b =>
                {
                    b.HasOne("WebApplication2.models.BudgetPlan", "BudgetPlan")
                        .WithMany("Items")
                        .HasForeignKey("BudgetPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WebApplication2.models.Commodity", "Commodity")
                        .WithMany()
                        .HasForeignKey("CommodityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BudgetPlan");

                    b.Navigation("Commodity");
                });

            modelBuilder.Entity("WebApplication2.models.MealSuggestion", b =>
                {
                    b.HasOne("WebApplication2.models.BudgetPlan", "BudgetPlan")
                        .WithMany("MealSuggestions")
                        .HasForeignKey("BudgetPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BudgetPlan");
                });

            modelBuilder.Entity("WebApplication2.models.ProductDietaryTag", b =>
                {
                    b.HasOne("WebApplication2.models.Commodity", "Commodity")
                        .WithMany("ProductDietaryTags")
                        .HasForeignKey("CommodityId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("WebApplication2.models.DietaryTag", "DietaryTag")
                        .WithMany("ProductDietaryTags")
                        .HasForeignKey("DietaryTagId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("Commodity");

                    b.Navigation("DietaryTag");
                });

            modelBuilder.Entity("WebApplication2.models.ProductPrice", b =>
                {
                    b.HasOne("WebApplication2.models.Commodity", "Commodity")
                        .WithMany("Prices")
                        .HasForeignKey("CommodityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WebApplication2.models.Market", null)
                        .WithMany("Prices")
                        .HasForeignKey("MarketId");

                    b.HasOne("WebApplication2.models.PriceReport", "Report")
                        .WithMany("Prices")
                        .HasForeignKey("ReportId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Commodity");

                    b.Navigation("Report");
                });

            modelBuilder.Entity("WebApplication2.models.UserFavorite", b =>
                {
                    b.HasOne("WebApplication2.models.Commodity", "Commodity")
                        .WithMany("FavoritedByUsers")
                        .HasForeignKey("CommodityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WebApplication2.models.User", "User")
                        .WithMany("Favorites")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Commodity");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WebApplication2.models.BudgetPlan", b =>
                {
                    b.Navigation("Items");

                    b.Navigation("MealSuggestions");
                });

            modelBuilder.Entity("WebApplication2.models.Commodity", b =>
                {
                    b.Navigation("FavoritedByUsers");

                    b.Navigation("Prices");

                    b.Navigation("ProductDietaryTags");
                });

            modelBuilder.Entity("WebApplication2.models.DietaryTag", b =>
                {
                    b.Navigation("ProductDietaryTags");
                });

            modelBuilder.Entity("WebApplication2.models.Market", b =>
                {
                    b.Navigation("Prices");
                });

            modelBuilder.Entity("WebApplication2.models.PriceReport", b =>
                {
                    b.Navigation("Prices");
                });

            modelBuilder.Entity("WebApplication2.models.User", b =>
                {
                    b.Navigation("BudgetPlans");

                    b.Navigation("Favorites");
                });
#pragma warning restore 612, 618
        }
    }
}
