...
                    else
                    {
                        var latestMonth = new DateTime(today.Year, today.Month, 1, 0, 0, 0, DateTimeKind.Utc);
                        var targetStart = latestMonth.AddMonths(-3); 
                        
                        result = groupedByMonth
                            .Where(g => g.Period >= targetStart)
                            .ToList();
                    }
                    break;
...
                    result = data
                        .Where(p => p.DateReported.Date >= startOfLast4Weeks)
...
                    break;
                default: 
                    var startOfCurrent7Days = today.AddDays(-6);
                    
                    var rawGroupedData = data
                        .Where(p => p.DateReported.Date >= startOfCurrent7Days)
...
                        .ToList(); 
                    ...
                    {
                        var uniquePeriods = Enumerable.Range(0, 7)
                            .Select(i => today.AddDays(-i).Date)
                            .OrderBy(d => d)
                            .Select(d => DateTime.SpecifyKind(d, DateTimeKind.Utc))
                            .ToList();

                        var uniqueCategories = rawGroupedData.Select(d => d.Category).Distinct().ToList();

                        result = uniquePeriods
                            .SelectMany(period => uniqueCategories.Select(category => new { Period = period, Category = category }))
                            .GroupJoin(rawGroupedData,
                                full => new { full.Period, full.Category },
                                raw => new { raw.Period, raw.Category },
                                (full, rawGroup) => new
                                {
                                    full.Period,
                                    full.Category,
                                    AvgPrice = rawGroup.DefaultIfEmpty().First()?.AvgPrice ?? 0
                                })
                            .OrderBy(d => d.Period)
                            .ToList();
                    }
...
            var result = data
                .Where(p => p.DateReported.Date >= startOfLastWeeks)
                .GroupBy(p => new
                {
